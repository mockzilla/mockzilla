...
1. **Config Override Middleware** - Applies per-request config overrides from `X-Mockzilla-*` headers
...
|--------|--------|-------------|
| `X-Mockzilla-Cache-Requests` | `true` / `false` | Enable/disable request caching |
| `X-Mockzilla-Latency` | Duration (e.g., `100ms`, `1s`) | Override latency |
| `X-Mockzilla-Upstream-Url` | URL or empty string | Override upstream URL (empty disables upstream) |
| `X-Mockzilla-Replay` | `body:f1,f2;query:f3` or `f1,f2` (or empty) | Activate replay; optionally override match fields |
...
| Header | Values | Description |
|--------|--------|-------------|
| `X-Mockzilla-Source` | `generated`, `cache`, `upstream`, `replay` | Where the response came from |
| `X-Mockzilla-Duration` | Duration (e.g., `5.123ms`) | Total request processing time |
...
curl -H "X-Mockzilla-Cache-Requests: false" http://localhost:2200/petstore/pets

# Add 500ms latency
curl -H "X-Mockzilla-Latency: 500ms" http://localhost:2200/petstore/pets

# Disable upstream proxy (force mock response)
curl -H "X-Mockzilla-Upstream-Url: " http://localhost:2200/petstore/pets

# Redirect to a different upstream
curl -H "X-Mockzilla-Upstream-Url: https://api.example.com" http://localhost:2200/petstore/pets

# Combine multiple overrides
curl -H "X-Mockzilla-Latency: 200ms" -H "X-Mockzilla-Cache-Requests: true" http://localhost:2200/petstore/pets
```
...
- `x-mockzilla-cache-requests: false`
- `X-Mockzilla-Cache-Requests: false`
- `X-MOCKZILLA-CACHE-REQUESTS: false`
...
1. Request arrives with `X-Mockzilla-Replay` header (or hits an `auto-replay` endpoint)
2. Replay Read checks for a stored recording matching the request body fields
3. If found → return immediately with `X-Mockzilla-Source: replay`

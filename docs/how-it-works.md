...
1. **Config Override Middleware** - Applies per-request config overrides from `X-Mz-*` headers
...
|--------|--------|-------------|
| `X-Mz-Cache-Requests` | `true` / `false` | Enable/disable request caching |
| `X-Mz-Latency` | Duration (e.g., `100ms`, `1s`) | Override latency |
| `X-Mz-Upstream-Url` | URL or empty string | Override upstream URL (empty disables upstream) |
| `X-Mz-Replay` | `body:f1,f2;query:f3` or `f1,f2` (or empty) | Activate replay; optionally override match fields |
...
| Header | Values | Description |
|--------|--------|-------------|
| `X-Mz-Source` | `generated`, `cache`, `upstream`, `replay` | Where the response came from |
| `X-Mz-Duration` | Duration (e.g., `5.123ms`) | Total request processing time |
...
curl -H "X-Mz-Cache-Requests: false" http://localhost:2200/petstore/pets

# Add 500ms latency
curl -H "X-Mz-Latency: 500ms" http://localhost:2200/petstore/pets

# Disable upstream proxy (force mock response)
curl -H "X-Mz-Upstream-Url: " http://localhost:2200/petstore/pets

# Redirect to a different upstream
curl -H "X-Mz-Upstream-Url: https://api.example.com" http://localhost:2200/petstore/pets

# Combine multiple overrides
curl -H "X-Mz-Latency: 200ms" -H "X-Mz-Cache-Requests: true" http://localhost:2200/petstore/pets
```
...
1. Request arrives with `X-Mz-Replay` header (or hits an `auto-replay` endpoint)
2. Replay Read checks for a stored recording matching the request body fields
3. If found → return immediately with `X-Mz-Source: replay`
